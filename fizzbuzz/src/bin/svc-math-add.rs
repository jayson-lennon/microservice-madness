...
Error + Send + Sync + 'static >>
{
    ...
    = rand :: thread_rng() ; loop
    {
        ...
        default() ; broker :: add_endpoint("add", & bind, & service_client) .
        await ? ; let mut app = tide :: Server ::
        with_state(State { client : service_client, }) ; app . at("/") .
        post(recv_request) ; if app . listen(bind) . await . is_err()
        { continue ; } else { break ; }
    } Ok(())
}